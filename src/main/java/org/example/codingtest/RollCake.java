...
public class RollCake {
    ...
    public int solution(int[] topping) {
        ...
        Map<Integer, Integer> rightMap = new HashMap<>(); // 형
        Set<Integer> leftSet = new HashSet<>(); // 동생

        for (int a : topping) {
            rightMap.put(a, rightMap.getOrDefault(a, 0) + 1); // 전체토핑
        }

        for (int i=0; i<topping.length; i++) {
            int t = topping[i];

            leftSet.add(t); // 동생꺼에 추가

            rightMap.put(t, rightMap.get(t)-1); // 형꺼 하나 빼기
            if (rightMap.get(t) == 0) {
                rightMap.remove(t); // 0이면 삭제
            }
