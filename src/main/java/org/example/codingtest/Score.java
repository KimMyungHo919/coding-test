...
public class Score {
    ...
    public int[] solution(int k, int[] score) {
        List<Integer> hall = new ArrayList<>(); // 명예의전당
        int[] answer = new int[score.length];

        for (int i = 0; i < score.length; i++) {
            hall.add(score[i]); // 스코어 저장
            Collections.sort(hall, Collections.reverseOrder()); // 역순으로 정렬

            if (hall.size() > k) {
                hall.remove(k); // k번째 이후의 요소 제거. (최하위 숫자 제거)
            }

            answer[i] = hall.get(hall.size() - 1); // 최하위 점수 저장
        }
